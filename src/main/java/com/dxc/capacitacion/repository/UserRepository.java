...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query(value = "select id, nombre, primerApellido, segundoApellido from User u1_0 where nombre=:nombre")
    User findByName2(String nombre);

    List<User> findByNombre(String nombre);

    // List<User> findByPrimerApellidoAndSegundoApellido(String primerApellido);
}
